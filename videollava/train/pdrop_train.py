...
class TrainingArguments(transformers.TrainingArguments):
    ...
    dominant_num: int = field(default=20)
    context_num: int = field(default=7)
    ...
    monitor_grads_every: int = 100
    raise_on_nan_grad: bool = False
    dump_first_backward_grads: bool = False
    dump_first_backward_grads_path: Optional[str] = None
...
def train(attn_implementation=None):
    ...
    model = visionzip(model, dominant=training_args.dominant_num, contextual=training_args.context_num).to(training_args.device)

    # Initialize PDrop scheduling parameters on the inner model
    # try:
    #     layer_list = ast.literal_eval(model_args.layer_list) if model_args.layer_list is not None else []
    # except Exception:
    #     layer_list = []
    # try:
    #     image_token_ratio_list = ast.literal_eval(model_args.image_token_ratio_list) if model_args.image_token_ratio_list is not None else []
    # except Exception:
    #     image_token_ratio_list = []

    # Ensure ratios list has a leading 1.0 to support indexing [cur] and [cur+1]
    # if isinstance(image_token_ratio_list, list):
    #     image_token_ratio_list = [1.0] + image_token_ratio_list
    # else:
    #     image_token_ratio_list = [1.0]

    # If ratios are shorter than needed, pad with last value
    # required_len = len(layer_list) + 1
    # if len(image_token_ratio_list) < required_len:
    #     pad_val = image_token_ratio_list[-1] if len(image_token_ratio_list) > 0 else 1.0
    #     image_token_ratio_list = image_token_ratio_list + [pad_val] * (required_len - len(image_token_ratio_list))

    # Bind to inner Llama model for use in pdrop_forward
    # model.model.layer_list = model_args.layer_list
    # model.model.image_token_ratio_list = model_args.image_token_ratio_list
