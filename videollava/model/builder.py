...
def load_pretrained_model(model_path, model_base, model_name, pdrop_infer=False, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", **kwargs):
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            if 'mpt' in model_name.lower():
                ...
                print(f"Loading PDrop model from {model_path}")
                model = LlavaLlamaForCausalLM_PDrop.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
                print(f"PDrop model loaded")
            else:
                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
