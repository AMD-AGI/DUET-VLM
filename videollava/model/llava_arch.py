...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_videos(self, videos):  # [mini_b, c, t, h, w]
        # print(f">>> videos = Not visionzip")
        b, _, t, _, _ = videos.shape
        out = self.get_model().get_video_tower()(videos)  # [mini_b, t, n, c] or tuple
        video_features = out[0] if isinstance(out, (tuple, list)) else out
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                try:
                    print(f"[VLM] batch={batch_idx} vision_tokens: per_segment=[], total=0, final_seq_len={cur_input_embeds.shape[0]}")
                except Exception:
                    pass
                cur_image_idx += 1
                continue
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    # track number of vision tokens inserted for this segment
                    try:
                        vision_token_lens.append(int(cur_image_features.shape[0]))
                    except Exception:
                        pass
            ...
            # print concise per-sample vision token stats
            try:
                total_vision_tokens = int(sum(vision_token_lens)) if len(vision_token_lens) > 0 else 0
                print(f"[VLM] batch={batch_idx} vision_tokens: per_segment={vision_token_lens}, total={total_vision_tokens}, final_seq_len={cur_new_input_embeds.shape[0]}")
            except Exception:
                pass
    ...
    def prepare_inputs_labels_for_multimodal_pdrop(
        ...
        if (image_tower is None and video_tower is None) or images is None or input_ids.shape[1] == 1:
            # set safe defaults for pdrop attributes
            try:
                self.model.image_token_posi = [-1] * (batch_size or 1)
                self.model.prompt_len = None
                self.model.image_tokens = [0] * (batch_size or 1)
            except Exception:
                pass
            if past_key_values is not None and (image_tower is not None or video_tower is not None) and images is not None and input_ids.shape[1] == 1:
                target_shape = past_key_values[-1][-1].shape[-2] + 1
        ...
        # Split mixed images/videos into minibatches
        image_idx = [idx for idx, img in enumerate(images) if img.ndim == 3]
        video_idx = [idx for idx, vid in enumerate(images) if vid.ndim == 4]
        images_minibatch = torch.stack([images[idx] for idx in image_idx]) if len(image_idx) > 0 else []  # [mini_b, c, h, w]
        videos_minibatch = torch.stack([images[idx] for idx in video_idx]) if len(video_idx) > 0 else []  # [mini_b, c, t, h, w]
        ...
        if getattr(videos_minibatch, 'ndim', 0) == 5:
            # print(f">>> videos_minibatch = {videos_minibatch}")
            # print(f">>> videos_minibatch.isnan() = {videos_minibatch.isnan().any()}")
            if videos_minibatch.isnan().any():
                print(f">>> videos_minibatch is nan")
                raise ValueError("videos_minibatch is nan")
            video_features_minibatch = self.encode_videos(videos_minibatch).to(input_ids.device)  # [mini_b, t, l, c]
            # print(f">>> video_features_minibatch = {video_features_minibatch}")
            # print(f">>> video_features_minibatch.isnan() = {video_features_minibatch.isnan().any()}")
            if video_features_minibatch.isnan().any():
                print(f">>> video_features_minibatch is nan")
                raise ValueError("video_features_minibatch is nan")
            for i, pos in enumerate(video_idx):
                t = video_features_minibatch[i].shape[0]
                tmp_image_features[pos] = [video_features_minibatch[i][j] for j in range(t)]

        # Flatten so each placeholder maps to one feature block (video -> multiple blocks)
        image_features = []
        for feat in tmp_image_features:
            if isinstance(feat, list):
                image_features.extend(feat)
            else:
                image_features.append(feat)

        # Prepare masks and ids
        _labels = labels
        _position_ids = position_ids
        _attention_mask = attention_mask
        if attention_mask is None:
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # print(f">>> cur_input_ids_noim.isnan() = {cur_input_ids_noim.isnan().any()}")
            cur_input_embeds = self.get_model().embed_tokens(torch.cat(cur_input_ids_noim))
            # print(f">>> cur_input_embeds = {cur_input_embeds}")
            # print(f">>> cur_input_embeds.isnan() = {cur_input_embeds.isnan().any()}")
            cur_input_embeds_no_im = torch.split(cur_input_embeds, split_sizes, dim=0)
            # print(f">>> cur_input_embeds_no_im = {cur_input_embeds_no_im}")
            # print(f">>> cur_input_embeds_no_im.isnan() = {cur_input_embeds_no_im.isnan().any()}")
        ...
        # # bind pdrop-related attributes on the underlying model for later stages
        # try:
        #     self.model.image_token_posi = image_token_posi
        #     self.model.prompt_len = prompt_len
        #     self.model.image_tokens = image_tokens
        # except Exception:
        #     pass
        # print(f"new_input_embeds = {new_input_embeds}")
        if new_input_embeds.isnan().any():
            print(f">>> new_input_embeds is nan")
            raise ValueError("new_input_embeds is nan")
        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels
