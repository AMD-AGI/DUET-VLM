...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def pdrop_forward(
        ...
        if hidden_states.isnan().any():
            print(f">>> hidden_states is nan at layer {layer_idx}")
            raise ValueError("hidden_states is nan at layer")
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            # print(f">>> Layer Idx: {layer_idx} with Hidden states shape = {hidden_states.shape}")
            if output_hidden_states:
                all_hidden_states += (hidden_states,)
    ...
    def pdrop_rank_drop(
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":
            ...
            for i in range(batch_size):
                ...
                # print(f">>> Total Image tokens = {image_tokens[i]}")
                if image_index == -1:
                ...
                # print(f">>> Rank and drop layer: {rank_layer}")
                # print(f">>> Features shape: {features[i].shape}")
                # # print(f">>> Total tokens (text + vision): {image_tokens[i] + self.prompt_len[i]}")
                # print(f">>> Image index: {image_index}")
                # print(f">>> Vision Tokens before rank and drop: {image_tokens[i]}")
                # print(f">>> Vision Tokens after rank and drop: {keep_length[i]}\n\n")
